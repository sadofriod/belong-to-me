(function() {
	...
	mui.plusReady(function() {
		...
		localStorage.massageNum = 0+'data';
		localStorage.imgNum = 0+'url'
	});
	/**
	 *提交成功之后，恢复表单项 
	 */
	feedback.clearForm = function() {
		feedback.question.innerHTML = '';
...
//				console.log("event:"+e);
	...
	feedback.submitBtn.addEventListener('tap', function(event) {
		if (feedback.question.innerHTML) {
			var ma = feedback.question.innerHTML;
			var mg = feedback.files;
			mg = JSON.stringify(mg)
			localStorage[localStorage.massageNum] = ma;
			localStorage[localStorage.imgNum] = mg;
		}
		if (feedback.question.innerHTML.length > 2000) {
			return mui.toast('信息超长,请重新填写~')
		}
		if ((!feedback.question.innerHTML || feedback.question.innerHTML == '')&&feedback.files.length==0)
		{
			return mui.toast('内容不能为空')
		}
